...
public final class EmbeddedEngine implements DebeziumEngine<SourceRecord> {
    ...
    /**
     * An optional advanced field that specifies the maximum number of consecutive retriable polling
     * failures the engine will tolerate before stopping. Use a negative value for infinite retries.
     */
    ...
    protected static final Field.Set ALL_FIELDS = CONNECTOR_FIELDS.with(OFFSET_STORAGE, OFFSET_STORAGE_FILE_FILENAME,
            ...
            INTERNAL_KEY_CONVERTER_CLASS, INTERNAL_VALUE_CONVERTER_CLASS, MAX_RETRIES);
        ...
        if (runningThread.compareAndSet(null, Thread.currentThread())) {
            ...
            try {
                ...
                try {
                    ...
                    int retryCount = 0;
                    int maxRetryCount = config.getInteger(MAX_RETRIES);
                    try {
                        ...
                        while (runningThread.get() != null) {
                            ...
                            try {
                                ...
                                // Successful poll -> reset the consecutive retriable failure counter.
                                retryCount = 0;
                            }
                                ...
                                // A negative maxRetryCount means retry indefinitely (legacy behaviour).
                                if (maxRetryCount >= 0 && retryCount >= maxRetryCount) {
                                    LOGGER.error("Retriable exception thrown, but the maximum number of retries ({}) has been "
                                            + "reached; the connector will be stopped", maxRetryCount, e);
                                    throw e;
                                }
                                retryCount++;
                                LOGGER.info("Retriable exception thrown, connector will be restarted (retry {} of {})",
                                        retryCount, maxRetryCount, e);
